...
<body>
	...
	<div
		...
		<h3 class="display-4">Pending Candidate Approvals</h3>
		<br>
		<table class="table">
			...
			<sql:setDataSource var="db" driver="com.mysql.jdbc.Driver" url="jdbc:mysql://localhost:3306/jobportal?useTimeZone=true&serverTimezone=UTC" user="root" password="password" />

			<sql:query var="rs" dataSource="${db}">select cid, name, email, phone from candidates where authorized="Unauthorized"</sql:query>


			<tbody>

				<c:forEach items="${rs.rows}" var="can">
					<tr>
						<td><c:out value="${can.cid}"></c:out></td>
						<td><c:out value="${can.name}"></c:out></td>
						<td><c:out value="${can.email}"></c:out></td>
						<td><c:out value="${can.phone}"></c:out></td>
						<td><a href="AdminSetAuthoCand.jsp?${can.cid}">Authorize</a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
	...
	<div
		class="pricing-header px-3 py-3 pt-md-5 pb-md-4 mx-auto text-center">
		<a class="btn btn-primary btn-lg" href="AdminViewAllCand.jsp"
			role="button">View All Candidates</a>
	</div>
